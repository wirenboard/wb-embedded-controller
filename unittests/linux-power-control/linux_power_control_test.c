...
// ==================== suspend-to-off: пробуждение PMIC импульсом PWRON ====================

// Fix 1 (минимальная пауза «арм -> первый PWRON»): при мгновенном выходе из окна
// сна PWRON нельзя жать, пока с момента вооружения окна (armed_ts) не прошло
// WBEC_SUSPEND_WAKE_PWRON_MIN_DELAY_MS - PMIC ещё завершает вход в сон и глотает
// ранний импульс (стенд 2026-07-09). Проверяем границу: до паузы PWRON низкий,
// по её истечении - поднимается.
static void test_wake_pwron_min_delay_holds_pwron_until_armed_plus_delay(void)
{
    const systime_t armed_ts = 1000;

    utest_systick_set_time_ms(armed_ts);
    linux_cpu_pwr_seq_init(false);
    prepare_periodic_runtime(true, false);      // 5В есть, 3.3В нет (PMIC спит)

    // Мгновенный выход из окна: пробуждение с запросом минимальной паузы,
    // якорь - момент вооружения (armed_ts).
    linux_cpu_pwr_seq_wakeup();
    linux_cpu_pwr_seq_wake_pwron_min_delay(armed_ts);

    // За 1 мс до истечения паузы PWRON держим низким.
    utest_systick_set_time_ms(armed_ts + WBEC_SUSPEND_WAKE_PWRON_MIN_DELAY_MS - 1);
    linux_cpu_pwr_seq_do_periodic_work();
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0, utest_gpio_get_output_state(pmic_pwron_gpio),
        "PWRON must stay low until the arming-to-pulse min delay has elapsed"
    );

    // По истечении паузы (>= MIN_DELAY с момента арма) PWRON поднимается.
    utest_systick_set_time_ms(armed_ts + WBEC_SUSPEND_WAKE_PWRON_MIN_DELAY_MS);
    linux_cpu_pwr_seq_do_periodic_work();
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(pmic_pwron_gpio),
        "PWRON must be pressed once the arming-to-pulse min delay has elapsed"
    );
    TEST_ASSERT_TRUE_MESSAGE(linux_cpu_pwr_seq_is_busy(), "wake sequence must still be busy");
}

// Контроль: без запроса паузы (реальный сон, ticks > 0) пробуждение жмёт PWRON
// на первом же проходе - min-delay применяется ТОЛЬКО к мгновенному выходу.
static void test_wake_pwron_without_min_delay_presses_immediately(void)
{
    linux_cpu_pwr_seq_init(false);
    prepare_periodic_runtime(true, false);

    linux_cpu_pwr_seq_wakeup();                 // паузу не запрашиваем

    linux_cpu_pwr_seq_do_periodic_work();
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(pmic_pwron_gpio),
        "a wake without the min-delay request must press PWRON immediately"
    );
}

// Fix 2 (verify-and-retry): если 3.3В не возвращается, PWRON повторяется
// WBEC_SUSPEND_WAKE_PWRON_ATTEMPTS раз (~1 с проверка каждой попытки), после чего
// путь пробуждения проваливается в штатный бэкстоп (снятие 5В -> цикл включения).
static void test_wake_pwron_retries_then_falls_back_to_5v_reset(void)
{
    linux_cpu_pwr_seq_init(false);
    prepare_periodic_runtime(true, false);      // 3.3В не появится никогда

    linux_cpu_pwr_seq_wakeup();                 // без паузы -> жмём сразу

    // Попытка 1: PWRON нажат на первом проходе.
    linux_cpu_pwr_seq_do_periodic_work();
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(pmic_pwron_gpio), "attempt 1: PWRON must be pressed"
    );
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(linux_power_gpio), "5V must be on during PWRON retries"
    );

    // Каждая попытка: ~1 с без 3.3В -> PWRON отпущен (пауза), затем повторное
    // нажатие через паузу step3 (500 мс). Так дважды -> попытки 2 и 3.
    for (unsigned retry = 2; retry <= WBEC_SUSPEND_WAKE_PWRON_ATTEMPTS; retry++) {
        utest_systick_advance_time_ms(WBEC_SUSPEND_WAKE_PWRON_RETRY_MS + 1);
        linux_cpu_pwr_seq_do_periodic_work();
        TEST_ASSERT_EQUAL_UINT32_MESSAGE(
            0, utest_gpio_get_output_state(pmic_pwron_gpio),
            "PWRON must be released between retries"
        );
        ...
        TEST_ASSERT_EQUAL_UINT32_MESSAGE(
            1, utest_gpio_get_output_state(pmic_pwron_gpio),
            "PWRON must be pressed again on the next retry"
        );
        TEST_ASSERT_TRUE_MESSAGE(linux_cpu_pwr_seq_is_busy(), "sequence must stay busy across retries");
    }

    // Последняя попытка тоже без 3.3В: попытки исчерпаны -> бэкстоп снимает 5В.
    utest_systick_advance_time_ms(WBEC_SUSPEND_WAKE_PWRON_RETRY_MS + 1);
    linux_cpu_pwr_seq_do_periodic_work();
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0, utest_gpio_get_output_state(linux_power_gpio),
        "after all PWRON attempts fail the wake path must fall back to the 5V reset backstop"
    );
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0, utest_gpio_get_output_state(pmic_pwron_gpio),
        "PWRON must be released when falling back to the 5V reset"
    );
    TEST_ASSERT_TRUE_MESSAGE(linux_cpu_pwr_seq_is_busy(), "the 5V reset backstop keeps the sequence busy");
}

// Verify-and-retry, положительный случай: 3.3В возвращается после второго
// импульса (первый PMIC проглотил) - последовательность завершается штатно,
// без снятия 5В.
static void test_wake_pwron_completes_when_v33_returns_on_retry(void)
{
    linux_cpu_pwr_seq_init(false);
    prepare_periodic_runtime(true, false);
    ...
    linux_cpu_pwr_seq_do_periodic_work();       // попытка 1: PWRON нажат
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(pmic_pwron_gpio), "attempt 1: PWRON pressed"
    );

    // Первый импульс проглочен: ~1 с без 3.3В -> отпускаем, пауза step3.
    utest_systick_advance_time_ms(WBEC_SUSPEND_WAKE_PWRON_RETRY_MS + 1);
    linux_cpu_pwr_seq_do_periodic_work();
    utest_systick_advance_time_ms(501);
    linux_cpu_pwr_seq_do_periodic_work();       // попытка 2: PWRON нажат снова
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(pmic_pwron_gpio), "retry 2: PWRON pressed again"
    );

    // Теперь 3.3В вернулось: PWRON отпущен, 5В остаётся включённым.
    utest_vmon_set_ch_status(VMON_CHANNEL_V33, true);
    linux_cpu_pwr_seq_do_periodic_work();
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0, utest_gpio_get_output_state(pmic_pwron_gpio), "PWRON must be released once 3.3V returns"
    );
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        1, utest_gpio_get_output_state(linux_power_gpio), "5V must stay on when the retry succeeds"
    );
}
...
int main(void)
{
    ...
    // suspend-to-off: пробуждение PMIC импульсом PWRON (min-delay + verify-and-retry)
    RUN_TEST(test_wake_pwron_min_delay_holds_pwron_until_armed_plus_delay);
    RUN_TEST(test_wake_pwron_without_min_delay_presses_immediately);
    RUN_TEST(test_wake_pwron_retries_then_falls_back_to_5v_reset);
    RUN_TEST(test_wake_pwron_completes_when_v33_returns_on_retry);

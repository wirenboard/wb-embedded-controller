...
//
// PMIC на WB 7.4 - AXP221 (семейство AXP20x/22x), а не AXP15060 как на WB 8.5.
// У AXP20x/22x PWROK - это ВЫХОД (power-good / сброс для SoC; задержка выдачи
// настраивается REG 0x36, бит 2), и в этом семействе вообще нет функции
// "перезапуск PMIC по низкому уровню на PWROK" - она появилась только в
// AXP15060/AXP2101, где её и приходится глушить через REG32[4]=0.
// Поэтому глушить нечего: импульс сбрасывает только SoC, выходы PMIC остаются
// включёнными, питание DRAM сохраняется.
//
// Проверено на железе (WB 7.4): после тёплого сброса по watchdog
// (poweron_reason = 8) содержимое RTC GP регистров уцелело, а systemd-pstore
// собрал console-ramoops с логом, записанным ДО сброса, т.е. ramoops в DRAM
// пережил перезагрузку. На второй ступени эскалации (жёсткий сброс по питанию,
// reason 5) те же RTC регистры обнуляются - питание действительно снимается.
//
// Импульс (100 мс) короче, чем удержание этой же линии в уже используемом
// linux_cpu_pwr_seq_reset_pmic() (до 2 с), т.е. новых воздействий на железо
// не вносит.

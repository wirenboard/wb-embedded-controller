...
#define REGMAP(m) \
    ...
    /*     Addr     Name            RO/RW */ \
    /* TODO: Выделять по целый регистр там, где достаточно бита - это жир, но для тестов через debugfs так сильно удобнее... */ \
    m(     0x70,    LED,            RW, \
        /* 0x70 */  uint16_t state; /* Состояние свечения светодиода (0 - off, 1 - on) */ \
        /* 0x71 */  uint16_t delay_on; /* Время включения при мигании в мск */ \
        /* 0x72 */  uint16_t delay_off; /* Время выключения при мигании в мск */ \
        /* 0x73 */  uint16_t mode; /* Режим (аналоги тригеров): 0 - none, 1 - timer */ \
    ) \
    /*     Addr     Name            RO/RW */ \
    m(     0x80,    GPIO,           RW, \
        /* 0x80 */  uint16_t a1 : 1; \

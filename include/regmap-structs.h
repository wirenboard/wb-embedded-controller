...
#define REGMAP(m) \
    ...
    /*     Addr     Name            RO/RW */ \
    /* ВНИМАНИЕ: структуры regmap упакованы (packed), поэтому битфилды */ \
    /* занимают минимально возможное число байтов. Если после битфилдов */ \
    /* есть другие поля, слово нужно явно добить анонимным битфилдом */ \
    /* до 16 бит, иначе последующие поля сдвинутся относительно */ \
    /* регистровой сетки. */ \

...
// suspend-to-off: минимальная пауза между вооружением окна сна (≈ момент, когда
// BL31 снял рельсы PMIC / взвёл его сон) и первым импульсом PWRON пробуждения.
// Если окно выходит «мгновенно» (будильник уже ждал на входе — exit cause=ALARM
// ticks=0), PWRON успевал прилететь через ~0.1-0.2 с после того, как BL31 усыпил
// PMIC. PMIC ещё доводит вход в сон и глотает импульс: SoC не стартует, и разбудить
// PMIC уже нечем (стенд 2026-07-09, 3 отказа, простой ~4.5 мин по дедлайн-ладдеру).
// Ждём завершения перехода PMIC в сон перед первым PWRON. Отсчёт — от stop_armed_ts.
#define WBEC_SUSPEND_WAKE_PWRON_MIN_DELAY_MS    500
// suspend-to-off: verify-and-retry импульса PWRON. Если 3.3В не вернулось за это
// время после нажатия PWRON, PMIC проглотил импульс — жмём ещё раз, всего до
// WBEC_SUSPEND_WAKE_PWRON_ATTEMPTS попыток, затем штатный бэкстоп (снятие 5В ->
// полный цикл включения, дальше дедлайн-эскалация).
#define WBEC_SUSPEND_WAKE_PWRON_RETRY_MS        1000
#define WBEC_SUSPEND_WAKE_PWRON_ATTEMPTS        3

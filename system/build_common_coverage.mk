...
PROJECT_UNCOVERED_SRC_JSON = $(PROJECT_COVERAGE_REPORT_DIR)/uncovr_src.json
PROJECT_GCOVR_FORMAT_VERSION_FILE = $(PROJECT_COVERAGE_REPORT_DIR)/gcovr_format_version.json
...
PROJECT_COVERAGE_DEPS = remove_project_report_dir $(PROJECT_COVERAGE_REPORT_DIR) $(PROJECT_COVERAGE_TARGETS)
...
# Project-only coverage: handle uncovered files
ifneq ($(COVERAGE_NO_ADD_UNCOVERED_FILES),1)

# Add $(PROJECT_UNCOVERED_SRC_JSON) to dependencies for "coverage-project" target
PROJECT_COVERAGE_DEPS += $(PROJECT_UNCOVERED_SRC_JSON)
PROJECT_COVERAGE_ADD_TRACE_FILES += -a $(PROJECT_UNCOVERED_SRC_JSON)

# Generate JSON for uncovered source files (project-only)
$(PROJECT_UNCOVERED_SRC_JSON): $(PROJECT_UNCOVERED_SRC_LIST_FILE) $(PROJECT_GCOVR_FORMAT_VERSION_FILE)
	@echo "\nGenerating data file for uncovered source files (project-only)..."
#	Usage: coverage_helper.sh --gen-uncovered-json UNCOVR_SRC_FILE GCOVR_VER_JSON OUT_UNCOVR_SRC_JSON
	./system/coverage_helper.sh --gen-uncovered-json $< $(PROJECT_GCOVR_FORMAT_VERSION_FILE) $@
	@echo "Uncovered sources data file saved: $@"

# Generate auxiliary file with uncovered sources list (project-only)
...
# Generate an empty JSON trace data file for project-only coverage
$(PROJECT_GCOVR_FORMAT_VERSION_FILE):
	@gcovr -r $(PROJECT_COVERAGE_REPORT_DIR) --json-pretty -o $@
...
#	Usage: coverage_helper.sh --make-coverage TEST_DIR OUT_COVR_DATA_FILE [GCC_BIN]
	@if [ -f "$(COV_DIR)/Makefile" ]; then \
		./system/coverage_helper.sh --make-coverage $(COV_DIR) $(PROJECT_COVERAGE_DATA_LIST_FILE) $(GCC_BIN); \
	fi

# Create an empty file for project coverage data list
$(PROJECT_COVERAGE_DATA_LIST_FILE):
	@touch $@

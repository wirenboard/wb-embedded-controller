...
Для измерения покрытия кода тестами выполните команду:

```bash
make coverage
```

После выполнения команды будет сгенерирован отчет в формате HTML: `covr_report/coverage_report.html`. Отчет содержит информацию о покрытии по строкам, функциям и ветвлениям для каждого файла.

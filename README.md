...
Проект поддерживает юнит-тестирование и измерение покрытия кода тестами с использованием gcovr.

### Запуск юнит-тестов

Для запуска всех юнит-тестов выполните команду:
...
Для измерения покрытия кода всего проекта:

```bash
make coverage
```

Для измерения покрытия только кода проекта (без субмодулей):

```bash
make coverage-project
```

После выполнения команды будет сгенерирован отчет в формате HTML:
- `covr_report/coverage_report.html` - отчет для всего проекта
- `covr_report_project/coverage_report.html` - отчет только для кода проекта

Отчет содержит информацию о покрытии по строкам, функциям и ветвлениям для каждого файла.
